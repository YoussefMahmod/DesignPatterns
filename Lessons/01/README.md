...
احيانا في المشاريع الضخمة التى تمتلك مجموعة كبيرة من الكلاسات المترابطة مع بعضها البعض  بشكل متشابك  
يكون صعب ان تتبع البرنامج واحيانا يصعب التعديل عليه  
...
اي اننا نستخدمنها لوصف الكلاسات التى لدينا في المشروع وما المتغيرات التى يحتويه وما الدوال التى به وسواء كانت عامة او خاصة `private` ام `public` والعلاقات ما بين الكلاسات ببعضها البعض  
...
وان اردنا ان نضع شرطًا معينا لهذه المتغيرات، فمثلا ان نقول ان الـ `id` يجب ان يكون `unique` اي لا يمكن تكراره، او ان نقول ان الاسماء تكون متربة  
...
ولا يجب ان يكون له `object` لذا سنجعل كلاس الـ `Animal` يكون `Abstract Class` بجعل اسم الكلاس مائلًا قليلا  
...
هذا الكلاس نريده ان يرث كلاس الـ `Animal`  
...
والاعتمادية تظهر بوضوح تام مع الـ `interface` لان لو `كلاس أ` لديه علاقة `implement` مع `interface`  
فهكذا `كلاس أ` سيستخدم جميع دوال الـ `interface` بشكل اجباري  

![](Image/3.png)

هكذا الكلاس `WildAnimal` سيأخذ ويستخدم جميع الدوال بشكل اجباري من الـ `interface` الـ `Animal`  
فهكذا كلاس `WildAnimal` سيأخد الدالتين `setName` و`getName` عنده بشكل اجباري ليستخدمهم ويعدل عليهم  
...
وهذا اجباري على `الكلاس أ` فإن اختفى `ب` فسيختفي `أ`  
